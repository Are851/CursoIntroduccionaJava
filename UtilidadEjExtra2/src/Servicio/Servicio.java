...
letrasencontradas y la cantidadjugadas máximas que puede realizar el usuario.
...
public class Servicio {
    ...
    public void crearJuego(Ahorcado ah) {
        ...
        String palabrasecreta1 = leer.next();
        System.out.println("Ingrese la cantidad de Jugadas");
        ah.setCantidadjugadas(leer.nextInt());
        for (int i = 0; i < palabrasecreta1.length(); i++) {
            juego.setPalabrasecreta(i,palabrasecreta1.charAt(i));
        }
        for (int i = 0; i < palabrasecreta1.length(); i++) {
            System.out.println(Arrays.toString(juego.getPalabrasecreta()));
    }
